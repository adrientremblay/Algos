...
public class CoinSelectionProblem {
    public static int dpSolution(int values[], int total) throws Exception {
        ...
        for (int i = 0 ; i < m.length ; i++) {
            for (int j = 1 ; j < m[i].length ; j++) {
                if (i == 0) {
                    ...
                    m[i][j] = m[i - 1][j];
                } else if (m[i-1][j] == -1 && m[i][j-values[i]] == -1) {
                    throw new Exception("bruh");
                } else if (m[i-1][j] == -1 ) {
                    m[i][j] = m[i][j-values[i]] + 1;
                } else if (m[i][j-values[i]] == -1) {
                    m[i][j] = m[i - 1][j];
                } else {
    ...
    public static void main(String args[]) throws Exception {
        ...
        dpSolution(values, target);

        int[] values2 = {2, 5, 6, 8};
        int target2 = 11;

        dpSolution(values2, target2);
    }
}
